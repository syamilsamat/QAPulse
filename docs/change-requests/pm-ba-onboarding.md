...
- **New page** `artifacts/qa-pulse/src/pages/RequirementDetail.tsx` at route `/requirements/:id` — replaces "click a row → open edit modal" with a real detail view (see mockup shared in chat), containing:
  - **Back button** + a **clickable breadcrumb** (`Demo Project » Requirements » #143`) — every breadcrumb segment navigates back (to the project's filtered list, or the unfiltered Requirements list), not just decorative text.
