...
- **Schema**: add `reviewedBy integer` and `reviewedAt timestamp` to `requirementsTable`, **plus `createdBy integer`** — a field the table doesn't have today (it currently has no author-tracking column at all). `POST /requirements` populates it from the auth token the same way `test-cases.ts` already does for `authorId`.
...
- **New endpoint** `PATCH /requirements/:id/review` in `routes/requirements.ts`: restricted to any FA-track role (`functional_analyst`, `fa_lead`, `hod_fa_bi`) or `admin`/`cto`, also passes through the project-access check; **additionally rejects with 403 if `req.authUser.id === requirement.createdBy`** ("cannot approve your own requirement"); updates `status`/`reviewedBy`/`reviewedAt`, writes to `activityTable`, notifies the requirement's `assigneeId` via `notificationsTable`. This is the **upstream** approval gate.
...
- **New endpoint** `GET /requirements/:id/activity` for a "Review History" panel.
- `Requirements.tsx`: add approve/reject actions + comment box, visible for any FA-track role or admin/cto — **except the requirement's own author**, who sees a disabled/hidden state instead (e.g. "Awaiting review from another FA") rather than a clickable approve button on their own work. PM Dashboard's milestone cards get the same affordance for the UAT gate (milestones have no single "author" in the same sense, so no self-approval restriction applies there).
