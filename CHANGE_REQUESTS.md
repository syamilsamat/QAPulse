...
**Origin:** six user-reported items in one pass — a hard error blocking dev-task creation, two workflow gates, two execution-sheet readability gaps, and a Tasks-page visualization request — plus two more reported while the batch was open (items 7 and 8). Grouped as one CR because they ship together; each is independent and separately revertible.
