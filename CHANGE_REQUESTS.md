...
- **`milestones.completed_at` added** *(2026-07-05)*: authoritative timestamp for when a milestone actually finished, auto-stamped by `PATCH /milestones/:id` on the transition into `status = "completed"` (and cleared if it moves away again) — same pattern as `requirements.approvedAt`/`rejectedAt`. Built as groundwork for a PM Dashboard "where did the time go" phase-breakdown report (Requirements phase → unattributed gap before QA → QA testing phase, plus a trend average across a project's last several completed milestones) — the report itself isn't built yet, just this data-integrity prerequisite.
